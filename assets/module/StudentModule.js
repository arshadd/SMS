var StudentModule = function () {
    ...
    var loadGridUrl = baseApiUrl + "Student/List";
    var editUrl = baseApiUrl + "Student/Find/";
    var saveUrl = baseApiUrl + "Student/Save";
    var deleteUrl = baseApiUrl + "Student/Delete/";
    var listUrl = baseAppUrl + "Student/List";
    ...
    var handleValidation = function () {
        ...
        var handleTitle = function (tab, navigation, index) {
            ...
            var current = index + 1;
            // set wizard title
            $('.step-title', $('#Form_Student_Wizard')).text('Step ' + (index + 1) + ' of ' + total);
            // set done steps
            jQuery('li', $('#Form_Student_Wizard')).removeClass("done");
            ...
            if (current == 1) {
                $('#Form_Student_Wizard').find('.button-previous').hide();
            } else {
                $('#Form_Student_Wizard').find('.button-previous').show();
            }

            if (current >= total) {
                $('#Form_Student_Wizard').find('.button-next').hide();
                $('#Form_Student_Wizard').find('.button-submit').show();
                displayConfirm();
            } else {
                $('#Form_Student_Wizard').find('.button-next').show();
                $('#Form_Student_Wizard').find('.button-submit').hide();
            }
            App.scrollTo($('.page-title'));
        }

        // default form wizard
        $('#Form_Student_Wizard').bootstrapWizard({
            ...
            onTabShow: function (tab, navigation, index) {
                ...
                $('#Form_Student_Wizard').find('.progress-bar').css({
                    width: $percent + '%'
                });
            }
        });
        ...
        $('#Form_Student_Wizard').find('.button-previous').hide();
        $('#Form_Student_Wizard .button-submit').click(function () {
    ...
    function save() {
        ...
        student.StudentId = StudentIdFld.val();
        ...
        $.ajax({
            ...
            complete: function (result) {
                ...
                debugger;
                if (obj.Success == true) {
                    window.location = listUrl + "?message=Student Information Saved";
                } else {
    ...
    var loadGrid = function () {
        ...
        var newTable = $('#StudentGrid').DataTable({
            //dom: "Bfrtip",
            ajax: loadGridUrl,
            columns: [
                ...
                { data: "Status", defaultContent: "" },
                {
                    data: null, render: function (data, type, row) {
                        ...
                        //return '<a href="Edit/' + data.Student Id + '" class="btn btn-default btn-xs purple"><i class="fa fa-edit"></i> Edit</a>' +
                        //    '| <a href="#" class="btn btn-default btn-xs black"><i class="fa fa-trash-o"></i> Delete</a>';

                        return '<a href="Edit/' + data.StudentId + '" class="btn btn-default btn-xs purple"><i class="fa fa-edit"></i> Edit</a>';

                        //return '<a href="#" class="btn btn-default btn-xs purple editView" data-id="' + data.StudentId + '"><i class="fa fa-edit"></i> Edit</a>';
                    }
                },
    ...
    function showEdit(data) {
        ...
        StudentIdFld.val(data.StudentId);
