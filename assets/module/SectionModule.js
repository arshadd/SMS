var SectionModule = function () {
    ...
    //Create Section
    var Section = { SectionId: 0, Code: '', FirstName: '', LastName: '', DateOfBirth: '', Gender: '', DateOfJoining: '', DepartmentId: 0, DesignationId: 0, Qualification: '', TotalExperience: '', PresentAddress: '', PermanentAddress: '', Country: '', State: '', City: '', Mobile: '', Phone: '', Email: '', Photo: '', Status: '' };

    //Field Declaration Section
    
    var SectionIdFld = $('#SectionId');
    var CodeFld = $('#Code');
    var NameFld = $('#Name');
    var GradingSystemFld = $('#GradingSystem');
    

    var loadGridUrl = baseApiUrl + "Section/List";
    var editUrl = baseApiUrl + "Section/Find/";
    var saveUrl = baseApiUrl + "Section/Save";
    var deleteUrl = baseApiUrl + "Section/Delete/";
    var listUrl = baseAppUrl + "Section/List";
    ...
    var handleValidation = function () {
        ...
        form1.validate({
            ...
            errorSection: 'help-block', // default input error message class
            ...
            highlight: function (element) { // hightlight error inputs
                $(element)
                    .closest('.form-group').addSection('has-error'); // set error class to the control group
            },

            unhighlight: function (element) { // revert the change done by hightlight
                $(element)
                    .closest('.form-group').removeSection('has-error'); // set error class to the control group
            },

            success: function (label) {
                label
                    .closest('.form-group').removeSection('has-error'); // set success class to the control group
            },
    ...
    function save() {
        ...
        Section.SectionId = SectionIdFld.val();
        Section.Code = CodeFld.val();
        Section.Name = NameFld.val();
        Section.GradingSystem = GradingSystemFld.val();
        ...
        $.ajax({
            ...
            complete: function (result) {
                ...
                debugger;
                if (obj.Success == true) {
                    window.location = listUrl + "?message=Section Information Saved";
                } else {
    ...
    var loadGrid = function () {
        ...
        var newTable = $('#SectionGrid').DataTable({
            //dom: "Bfrtip",
            //ajax: loadGridUrl,
            columns: [
                ...
                { data: "Status" }, 
                {
                    data: null, render: function (data, type, row) {
                        ...
                        //return '<a href="Edit/' + data.Section Id + '" class="btn btn-default btn-xs purple"><i class="fa fa-edit"></i> Edit</a>' +
                        //    '| <a href="#" class="btn btn-default btn-xs black"><i class="fa fa-trash-o"></i> Delete</a>';

                        //return '<a href="Edit/' + data.SectionId + '" class="btn btn-default btn-xs purple"><i class="fa fa-edit"></i> Edit</a>';

                        return '<a href="#" class="btn btn-default btn-xs purple editView" data-id="' + data.SectionId + '"><i class="fa fa-edit"></i> Edit</a>';
                    }
                },
            ...
            "fnInitComplete": function () {
                $(".editView").click(function () {
                    ...
                    $('#mdlCreateSection').modal('show');
                });
            }
        });

        jQuery('#SectionGrid_wrapper .dataTables_filter input').addClass("form-control input-small"); // modify table search input
        jQuery('#SectionGrid_wrapper .dataTables_length select').addClass("form-control input-small"); // modify table per page dropdown
        jQuery('#SectionGrid_wrapper .dataTables_length select').select2(); // initialize select2 dropdown
    }
    ...
    function showEdit(data) {
        ...
        SectionIdFld.val(data.SectionId);
    ...
    function showPopup() {
        ...
        $('.modal-title').html("Create Section");
        $('#mdlCreateSection').modal('show');
    }
    //var view = function () {
    ...
    //    SectionIdFld.text(data.SectionId);
    ...
    //    $(".edit").attr("href", "../Edit/" + data.SectionId);
    //};
    ...
    //            if (obj.Success == true) {
    //                window.location = listUrl + "?message=Section Information Deleted";
    //            } else {
    //                ShowMessage("error", obj.Message);
    //            }
    //        }
